...
export const NON_VALVE_SUFFIXES = [
...
export function isDemoValve(entityId: string): boolean {
  return entityId.includes("_demonstration_");
}

export function isNonValveEntity(entityId: string): boolean {
  if (/^(?:number\.)?luftator_app_/i.test(entityId)) return true;
  if (isDemoValve(entityId)) return true;

  const normalizedId = entityId.toLowerCase().replace(/(?:_\d+)+$/, "");

  return NON_VALVE_SUFFIXES.some((suffix) => normalizedId.endsWith(suffix));
}

export function isValveEntity(entityId: string): boolean {
  // Must start with number.luftator_ or luftator_
  const hasValidPrefix = /^(?:number\.)?luftator_/i.test(entityId);
  if (!hasValidPrefix) return false;

  return !isNonValveEntity(entityId);
}
...
export class ValveManager implements ValveController {
  ...
  private isDemoValve(entityId: string): boolean {
    return isDemoValve(entityId);
  }

  private isValveEntity(entityId: string): boolean {
    return isValveEntity(entityId);
  }
