...
- `npm run format` / `npm run format:check` – Format/check code with Prettier
...
- **Local full-stack development**: 
  1. In root: `npm run dev` (Vite frontend)
  2. In `addon/rootfs/usr/src/app`: `npm run dev` (backend)
...
- **Testing**: `npm test` from root or `addon/rootfs/usr/src/app`
...
### Frontend (browser, via VITE_* prefix)
